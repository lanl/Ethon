...
#ifndef ETHON_BLOCK_MESH_AMR_DRIVER_HPP_
#define ETHON_BLOCK_MESH_AMR_DRIVER_HPP_

#include <chrono>
#include <memory>
#include <vector>

#include "block_mesh/amr.hpp"
...
public:
  ...
  AMRDriver(std::string out_file = "") : block_pool_(512) {
    if (out_file == "")
      f_ = stdout;
    else
      f_ = fopen(out_file.c_str(), "w");
  }
   ...
   */
  void ReplaceBlocksTask(const AMR::ReplaceTaskArgs<AMRDriver> &args) {
    mesh_changed_ = true;

    if (args.outgoing.size() == 1) {
      // this is a refinement, one block gets split into 8
      my_assert(args.incoming.size() == P8EST_CHILDREN,
          "Expected 8 children for refinement in ReplaceBlocks");
      auto parent = args.outgoing[0];

      for (int c = 0; c < P8EST_CHILDREN; ++c) {
        auto child = args.incoming[c];
        // The children should be in Morton ordering (i.e. x moves faster than y, which moves faster
        // than z, so the order for xyz is 000, 100, 010, 110, 001, 101, 011, 111).
        // Get child indices (0 or 1) in x, y, and z directions
        int ci = (c & (1 << 0)) == 0 ? 0 : 1;
        int cj = (c & (1 << 1)) == 0 ? 0 : 1;
        int ck = (c & (1 << 2)) == 0 ? 0 : 1;

        // prolongate interior data and ghost zones onto the child, the child will have all ghost
        // zones filled
        parent->ProlongateOntoChild(child, ci, cj, ck);
      }

      // destroy parent
      block_pool_.Free(parent);
    } else if (args.outgoing.size() == P8EST_CHILDREN) {
      // this is a coarsening of 8 blocks into 1

      my_assert(args.incoming.size() == 1,
          "Expected 1 new incoming block when coarsening in ReplaceBlocks");
      auto parent = args.incoming[0];

      for (int c = 0; c < P8EST_CHILDREN; ++c) {
        auto child = args.outgoing[c];
        // The children should be in Morton ordering (i.e. x moves faster than y, which moves faster
        // than z, so the order for xyz is 000, 100, 010, 110, 001, 101, 011, 111).
        // Get child indices (0 or 1) in x, y, and z directions
        int ci = (c & (1 << 0)) == 0 ? 0 : 1;
        ...
        // restrict
        child->RestrictOntoParent(parent, ci, cj, ck);

        // destroy child
        block_pool_.Free(child);
      }
    } else {
      my_assert(false, "Expected 1 or 8 outgoing blocks in ReplaceBlocks");
    }
  }
  ...
  void UpdateBlocksFaces() {
    ...
    if (mesh_changed_) {
      blocks_faces_.Clear();
      blocks_faces_.blocks.reserve(p8est_->global_num_quadrants);

      AMR::GetBlocksAndFaces<AMRDriver>(p8est_, &blocks_faces_);

      fprintf(f_, "%lu blocks, %lu same-level faces, %lu inter-level faces\n",
          blocks_faces_.blocks.size(),
          blocks_faces_.same_level_faces.size(),
          blocks_faces_.inter_level_faces.size());
    }

    time_p4est_iter_ += Elapsed(start);
  }

  void DoAMR(const typename Block_t::refinement_func_t refinement_func,
      const double time,
...
#pragma omp parallel for
      for (auto &b : blocks_faces_.blocks)
        b->EvaluateRefinement(refinement_func, time, amr_derefinement_count);
    }
    ...
    {
#pragma omp parallel for
      for (auto &arg : refine_coarsen_task_list)
        ReplaceBlocksTask(arg);

#pragma omp parallel for
      for (auto &arg : balance_task_list)
        ReplaceBlocksTask(arg);
    }
  ...
  void DoGhostExchange() {
    ...
    {
#pragma omp parallel for
      for (auto &f : blocks_faces_.same_level_faces)
        f->ExchangeGhostData();
    }
    ...
    {
#pragma omp parallel for
      for (auto &f : blocks_faces_.inter_level_faces)
        f->RestrictGhostData();
    }
...
#pragma omp parallel for
  ...
  void Evolve(const Mesh &mesh,
      ...
      const EOS &eos,
      const double cfl,
      const typename Block_t::refinement_func_t refinement_func,
      const int amr_max_level,
      const int amr_derefinement_count,
    ...
    while (mesh_changed_) {
      DoAMR(refinement_func, t_start, amr_max_level, amr_derefinement_count);
      if (mesh_changed_) {
        if (!initial_data->is_grid_data())
          ApplyInitialDataGenerator(static_cast<const GeneratorInitialData *>(initial_data));
...
#pragma omp parallel for reduction(min : new_dt)
      ...
      fprintf(f_, "[%06lu] t = %.10e, dt = %.10e\n", num_steps, t, dt);

      // if we're done, we don't need to do AMR
      if (t >= t_end) break;

      // Do AMR
      DoAMR(refinement_func, t, amr_max_level, amr_derefinement_count);
  ...
  void DumpGrid(const std::string &fname,
    ...
    output::DumpGrid(fname, num_steps, time, mesh, eos, blocks_faces_.blocks, z_slice);
  }
...
#endif // ETHON_BLOCK_MESH_AMR_DRIVER_HPP_
