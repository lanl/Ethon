...
#ifndef ETHON_KOKKOS_CUDA_OUTPUT_HPP_
#define ETHON_KOKKOS_CUDA_OUTPUT_HPP_
...
void DumpBlock(const typename BLOCK::CellData_t::HostMirror &dat,
  const BLOCK *block,
    double *data,
    double *x1f,
    double *x2f,
  ...
  for (size_t k = N_ghost - 1; k < N + N_ghost + 1; ++k) {
    for (size_t j = N_ghost - 1; j < N + N_ghost + 1; ++j) {
      for (size_t i = N_ghost - 1; i < N + N_ghost + 1; ++i) {
        ...
        if (num_outside <= 1) {
          double press = eos.pressure(BLOCK::GetState(dat, block->id(), i, j, k));
  ...
  for (size_t K = 0; K < N; ++K) {
    for (size_t J = 0; J < N; ++J) {
      for (size_t I = 0; I < N; ++I) {
        ...
        auto s = BLOCK::GetState(dat, block->id(), i, j, k);
...
void DumpGrid(const std::string &fname,
    ...
    const Mesh_t &mesh,
    const EOS &eos,
    const typename BLOCK::CellData_t::HostMirror &dat,
    const std::vector<BLOCK *> &all_blocks,
    const bool z_slice) {
  auto h5_fname = fname + ".athdf";
...
#pragma omp parallel for reduction(max : max_level)
    for (size_t i = 0; i < Nb; ++i) {
      DumpBlock(dat, out_blocks[i], data, x1f, x2f, x3f, levels, i, Nb, eos, z_slice);
      max_level = std::max(max_level, out_blocks[i]->level());
    }

    H5::H5File file(h5_fname, H5F_ACC_TRUNC);
...
#endif // ETHON_KOKKOS_CUDA_OUTPUT_HPP_
