...
#ifndef ETHON_KOKKOS_OMP_AMR_HPP_
#define ETHON_KOKKOS_OMP_AMR_HPP_
...
static void CreateBlockWithGridInitialData(
  ...
  block->SetState(driver->CellData(),
...
struct BlocksFaces {
  ...
  Kokkos::View<IntercellFace<Block_t> *, Kokkos::HostSpace> same_level_faces;
  Kokkos::View<BoundaryFace<Block_t> *, Kokkos::HostSpace> boundary_faces;
  Kokkos::View<InterLevelFace<Block_t, DRIVER::Slope> *, Kokkos::HostSpace> inter_level_faces;
};
...
static void GetBlocksAndFaces(p8est_t *p8est, BlocksFaces<DRIVER> *blocks_faces) {
  ...
  for (size_t i = 0; i < vecs.blocks.size(); ++i) {
    blocks_faces->block_ids(i) = vecs.blocks[i]->id();
    blocks_faces->cell_sizes(i, 0) = vecs.blocks[i]->cell_size()[0];
    blocks_faces->cell_sizes(i, 1) = vecs.blocks[i]->cell_size()[1];
    blocks_faces->cell_sizes(i, 2) = vecs.blocks[i]->cell_size()[2];
  }

  Kokkos::resize(blocks_faces->same_level_faces, vecs.same_level_faces.size());
  for (size_t i = 0; i < vecs.same_level_faces.size(); ++i)
    blocks_faces->same_level_faces(i) = vecs.same_level_faces[i];

  Kokkos::resize(blocks_faces->boundary_faces, vecs.boundary_faces.size());
  for (size_t i = 0; i < vecs.boundary_faces.size(); ++i)
    blocks_faces->boundary_faces(i) = vecs.boundary_faces[i];

  Kokkos::resize(blocks_faces->inter_level_faces, vecs.inter_level_faces.size());
  for (size_t i = 0; i < vecs.inter_level_faces.size(); ++i)
    blocks_faces->inter_level_faces(i) = vecs.inter_level_faces[i];
}

} // namespace AMR

#endif // ETHON_KOKKOS_OMP_AMR_HPP_
