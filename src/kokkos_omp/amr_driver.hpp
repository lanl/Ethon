...
#ifndef ETHON_KOKKOS_OMP_AMR_DRIVER_HPP_
#define ETHON_KOKKOS_OMP_AMR_DRIVER_HPP_

#include <chrono>
#include <memory>
#include <vector>

#include "kokkos_omp/amr.hpp"
#include "kokkos_omp/amr_mesh_info.hpp"
#include "kokkos_omp/block.hpp"
#include "kokkos_omp/block_pool.hpp"
#include "kokkos_omp/face.hpp"
#include "kokkos_omp/output.hpp"

template <typename RIEMANN, SlopeType SLOPE, size_t BLOCK_SIZE, typename ExecSpace>
class AMRDriver {
public:
  ...
  auto refinement_flag(const Block_t *block) const {
    return block_pool_.refinement_flag(block->id());
  }
  ...
  void DoReplaceBlocksTask(const typename Block_t::CellData_t &cell_data,
    ...
    Kokkos::View<AMR::ReplaceTaskArgs<AMRDriver> *, Kokkos::HostSpace> args_view(
...
#pragma omp parallel for
    ...
    // FIXME
    Kokkos::parallel_for(
        Kokkos::RangePolicy<ExecSpace>(0, args_view.extent(0)), KOKKOS_LAMBDA(int i) {
          if (args_view(i).restrict) {
            ...
            for (int c = 0; c < P8EST_CHILDREN; ++c) {
              ...
              Block_t::RestrictFieldOntoParent(
                  cell_data, args_view(i).coarse_bid, args_view(i).fine_bids[c], ci, cj, ck);
            }
          } else {
            ...
            for (int c = 0; c < P8EST_CHILDREN; ++c) {
              ...
              Block_t::ProlongateFieldOntoChild(
                  cell_data, args_view(i).fine_bids[c], args_view(i).coarse_bid, ci, cj, ck);
            }
          }
        });
...
#pragma omp parallel for
  ...
  void DoAMR(const typename Block_t::refinement_func_t refinement_func,
    ...
    Block_t::EvaluateRefinement(block_pool_.cell_data,
  ...
  void Evolve(const Mesh &mesh,
    ...
    while (mesh_changed_) {
      DoAMR(refinement_func, t_start, amr_threshold, eos, amr_max_level);
      if (mesh_changed_) {
        if (!initial_data->is_grid_data())
          ApplyInitialDataGenerator(static_cast<const GeneratorInitialData *>(initial_data));
    ...
    while (t < t_end) {
      ...
      // Do AMR
      DoAMR(refinement_func, t, amr_threshold, eos, amr_max_level);
  ...
  void DumpGrid(const std::string &fname,
    ...
    output::DumpGrid(
        fname, num_steps, time, mesh, eos, block_pool_.cell_data, blocks_faces_.blocks, z_slice);
  }
...
#endif // ETHON_KOKKOS_OMP_AMR_DRIVER_HPP_
