...
#include "kokkos_cuda/amr_driver.hpp"
...
void RunKH_Block_AMR(const std::string &output_prefix,
  ...
  AMRDriver<RIEMANN, SLOPE, BLOCK_SIZE, RefineCond::KH, Kokkos::Cuda> driver;
      ...
      for (uint j = 0; j < BLOCK_SIZE; ++j) {
        for (uint k = 0; k < BLOCK_SIZE; ++k) {
          ...
          block->SetStateLogicalIndices(
              driver.HostCellData(), i, j, k, State::FromPrimitive(rho, {ux, uy, uz}, p0, eos));
        }
      }
    }
  };
...
TEST(AMR, Kokkos_CUDA_KH_8) {
  RunKH_Block_AMR<HLLC, SlopeType::MC, 8>("amr_kokkos_CUDA_mc_8", 256, 128, 8, 0, 0.5);
}

void RunBlast_Block_AMR() {
  const int BLOCK_SIZE = 8;
  const std::string output_prefix = "amr_kokkos_CUDA_blast_8";
  ...
  BoundaryConditionType BCs[3][2] = {{OF, OF}, {OF, OF}, {OF, OF}};
  AMRDriver<HLLC, SlopeType::MC, BLOCK_SIZE, RefineCond::Blast, Kokkos::Cuda> driver;
  using Block_t = typename decltype(driver)::Block_t;
    ...
    for (int i = 0; i < BLOCK_SIZE; ++i) {
      for (int j = 0; j < BLOCK_SIZE; ++j) {
        for (int k = 0; k < BLOCK_SIZE; ++k) {
          ...
          block->SetStateLogicalIndices(
              driver.HostCellData(), i, j, k, State::FromPrimitive(rho, {ux, uy, uz}, p, eos));
        }
      }
    }
  };
...
TEST(AMR, Kokkos_CUDA_Blast_8) { RunBlast_Block_AMR(); }
